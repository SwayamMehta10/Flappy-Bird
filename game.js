    ...
    bubbles.forEach(bubble => {
        ...
        if (ctx.measureText(text).width <= maxWidth) {
            ...
            for (let i = 1; i < words.length; i++) {
                ...
                let diff = Math.abs(ctx.measureText(line1).width - ctx.measureText(line2).width);
                if (diff < minDiff) {
                    minDiff = diff;
                    bestSplit = i;
                }
        ...
        ctx.font = `bold ${fontSize}px Arial`;
        if (lines.length === 1) {
            ctx.fillText(lines[0], bubble.x, bubble.y + fontSize/8); // nudge down for better centering
        } else {
            // Center both lines in the bubble with a little extra gap
...
function render() {
    ...
    if (gameState === 'playing') {
        ...
        ctx.fillText(QUESTIONS[currentQuestionIdx % QUESTIONS.length].question, canvas.width/2, canvas.height - 30);
    }
